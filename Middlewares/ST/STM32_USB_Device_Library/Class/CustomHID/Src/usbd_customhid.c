  ...
  case USB_REQ_TYPE_CLASS :  
    ...
    case CUSTOM_HID_REQ_GET_REPORT:
      state = ((USBD_CUSTOM_HID_ItfTypeDef *)pdev->pUserData)->GetFeature(req->wValue & 0xff,
                                            buffer, (uint8_t)(req->wLength), &len);
                                 ...
                                 uint16_t len)
{
  ...
  if (pdev->dev_state == USBD_STATE_CONFIGURED )
  {
    ...
    else if (hhid->state == CUSTOM_HID_BUSY) {
      //hhid->state = CUSTOM_HID_IDLE;
      return 1;
                              ...
                              uint8_t epnum)
{
  ...
  USBD_CUSTOM_HID_HandleTypeDef     *hhid = (USBD_CUSTOM_HID_HandleTypeDef*)pdev->pClassData;  
  /*
  if (hhid->Report_buf[0] == 0x0A) {
    hhid->state = CUSTOM_HID_BUSY;
    USBD_LL_Transmit (pdev, CUSTOM_HID_EPIN_ADDR, buffer2, 3);
    hhid->state = CUSTOM_HID_IDLE;
  }
  */
  ...
  //if (hhid->Report_buf[0] == 0x0A) {//AnswerFlag = 1;
    ...
    //USBD_CUSTOM_HID_SendReport(&hUsbDeviceFS,buffer2,3);
        //hhid->state = CUSTOM_HID_BUSY;
        //HAL_PCD_EP_Transmit(pdev->pData, CUSTOM_HID_EPIN_ADDR, buffer2, 3);
        //USBD_LL_Transmit (pdev, CUSTOM_HID_EPIN_ADDR, buffer2, 3);
  ...
  */
  //((USBD_CUSTOM_HID_HandleTypeDef *)pdev->pClassData)->state = CUSTOM_HID_BUSY;
